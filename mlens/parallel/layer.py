    ...
    **kwargs : optional
    ...
    def __call__(self, args, parallel):
        ...
        if job != "fit" and not self.__fitted__:
            raise NotFittedError("Layer instance (%s) not fitted." % self.name)
        ...
        if self.verbose:
            ...
            safe_print(msg.format("Processing %s" % self.name), file=f, end=e1)
            t0 = time()

        if self.transformers:
            if self.verbose >= 2:
                safe_print(
                    msg.format("Preprocess pipelines ..."), file=f, end=e2
                )
                t1 = time()

            parallel(
                delayed(subtransformer, not _threading)()
        ...
        parallel(
            delayed(sublearner, not _threading)()
