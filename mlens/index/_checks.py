...
def check_full_index(n_samples, folds, raise_on_exception):
    ...
    if not isinstance(folds, Integral):
        raise ValueError(
            "'folds' must be an integer. " "type(%s) was passed." % type(folds)
        )

    if folds <= 1:
        if raise_on_exception:
            raise ValueError(
                "Need at least 2 folds to partition data. " "Got %i." % folds
            )
        else:
            if folds == 1:
                warnings.warn(
                    "'folds' is 1, will return full index as "
                    "both training set and test set."
                )
    if folds > n_samples:
        raise ValueError(
            "Number of splits %i is greater than the number "
...
def check_partial_index(n_samples, test_size, train_size, n_test, n_train):
    ...
    if n_test + n_train > n_samples:
        raise ValueError(
            "The selection of train (%r) and test (%r) samples "
            "lead to a subsets greater than the number of "
            "observations (%i). Implied test size: %i, "
            "implied train size: "
            "%i." % (test_size, train_size, n_samples, n_test, n_train)
        )

    for n, i, j in zip(
        ("test", "train"), (n_test, n_train), (test_size, train_size)
    ):
        if n == 0:
            raise ValueError(
                "The %s set size is 0 with current selection ("
                "%r): "
...
def check_subsample_index(n_samples, partitions, folds, raise_):
    ...
    if folds == 1:
        if raise_ or partitions > 1:
            raise ValueError(
                "Need at least 2 folds for splitting partitions. "
                "Got %i." % folds
            )
        else:
    ...
    if s > n_samples:
        raise ValueError(
            "Number of total splits %i is greater than the "
            "number of samples: %i." % (s, n_samples)
        )


def check_temporal_index(
    burn_in, step_size, window, lag, n_samples, raise_on_exception=None
):
    """Check that folds can be constructed from passed arguments."""
