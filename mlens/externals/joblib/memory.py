...
CacheItemInfo = collections.namedtuple(
...
class MemorizedResult(Logger):
    ...
    def __init__(
        ...
        self._output_dir = _cache_key_to_dir(
            cachedir, self.func, argument_hash
        )
        ...
        if metadata is not None:
            ...
            try:
                with open(
                    os.path.join(self._output_dir, "metadata.json"), "rb"
                ) as f:
    ...
    def __repr__(self):
        return (
            '{class_name}(cachedir="{cachedir}", func="{func}", '
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self.cachedir, self.func, self.argument_hash),
            {"mmap_mode": self.mmap_mode},
        )
...
class MemorizedFunc(Logger):
    ...
    def __init__(
        ...
        if compress and self.mmap_mode is not None:
            warnings.warn(
                "Compressed results cannot be memmapped", stacklevel=2
            )
    ...
    def _cached_call(self, args, kwargs):
        ...
        if not (
            ...
            and os.path.isfile(output_pickle_path)
        ):
            if self._verbose > 10:
                _, name = get_func_name(self.func)
                self.warn(
                    "Computing func %s, argument hash %s in "
    ...
    def _write_func_code(self, filename, func_code, first_line):
        ...
        if PY3_OR_LATER:
            is_named_callable = (
                hasattr(self.func, "__name__")
                and self.func.__name__ != "<lambda>"
            )
        else:
            is_named_callable = (
                hasattr(self.func, "func_name")
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        if self._verbose > 10:
            _, func_name = get_func_name(self.func, resolv_alias=False)
            self.warn(
                "Function %s (stored in %s) has changed."
                % (func_name, func_dir)
            )
    ...
    def _persist_output(self, output, dir):
        ...
        try:
            ...
            write_func = functools.partial(
                numpy_pickle.dump, compress=self.compress
            )
    ...
    def _persist_input(
        self, output_dir, duration, args, kwargs, this_duration_limit=0.5
    ):
...
class Memory(Logger):
    ...
    def __reduce__(self):
        ...
        return (
            self.__class__,
            (cachedir, self.mmap_mode, self.compress, self._verbose),
        )
