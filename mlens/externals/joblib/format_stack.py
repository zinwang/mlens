...
def format_exc(etype, evalue, etb, context=5, tb_offset=0):
    ...
    head = "%s%s%s\n%s%s%s" % (
        etype,
        " " * (75 - len(str(etype)) - len(date)),
        date,
        pid,
        " " * (75 - len(str(pid)) - len(pyver)),
        pyver,
    )
...
def format_outer_frames(
    context=5, stack_start=None, stack_end=None, ignore_ipython=True
):
    LNUM_POS, LINES_POS, INDEX_POS = 2, 4, 5
    records = inspect.getouterframes(inspect.currentframe())
    output = list()

    for i, (frame, filename, line_no, func_name, lines, index) in enumerate(
        records
    ):
        # Look inside the frame's globals dictionary for __file__, which should
        # be better.
