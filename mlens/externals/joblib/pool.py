    ...
    def __call__(self, a):
        ...
        if (
            ...
            if not os.path.exists(filename):
                ...
                print(
                    "Memmaping (shape=%s, dtype=%s) to old file %s"
                    % (a.shape, a.dtype, filename)
                )
            ...
            if self.verbose > 1:
                print(
                    "Pickling array (shape=%r, dtype=%s)." % (a.shape, a.dtype)
                )
...
class PicklingPool(Pool):
    ...
    def _setup_queues(self):
        ...
        self._inqueue = CustomizablePicklingQueue(
            context, self._forward_reducers
        )
        self._outqueue = CustomizablePicklingQueue(
            context, self._backward_reducers
        )
...
class MemmapingPool(PicklingPool):
    ...
    def __init__(
        ...
        **kwargs
        ...
        pool_folder, use_shared_mem = _get_temp_dir(
            pool_folder_name, temp_folder
        )
        ...
        if np is not None:
            ...
            forward_reduce_ndarray = ArrayMemmapReducer(
                max_nbytes, pool_folder, mmap_mode, verbose, prewarm=prewarm
            )
