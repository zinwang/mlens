...
def check_ensemble_build(inst, attr="stack"):
    ...
    if not hasattr(inst, attr):
        ...
        msg = (
            "No layer class attached to instance (%s). (Cannot find a "
    ...
    if getattr(inst, attr) is None:
        ...
        if not getattr(inst, "raise_on_exception", True):
            ...
            warnings.warn(
                msg % inst.__class__.__name__, LayerSpecificationWarning
            )
            ...
            msg = (
                "No Layers in instance (%s). Add layers before calling "
                "'fit' and 'predict'."
            )
...
def assert_valid_estimator(instance):
    ...
    try:
        ...
        raise TypeError(
            "'%s' does not appear to be an instance of an "
...
def assert_correct_format(estimators, preprocessing):
    ...
    if (preprocessing is None) or (not isinstance(preprocessing, dict)):
        if isinstance(estimators, dict):
            ...
            msg = (
                "Preprocessing is either 'None' or 'list': 'estimators' "
        ...
        if not isinstance(preprocessing, dict):
            msg = (
                "Unexpected format of 'preprocessing'. Needs to be "
        ...
        if not isinstance(estimators, dict):
            msg = (
                "Unexpected format of 'estimators'. Needs to be "
        ...
        if not all(est_check):
            msg = (
                "Too few estimator cases to match preprocessing cases:\n"
                "estimator cases:     %r\npreprocessing cases: %r"
            )
            raise LayerSpecificationError(
                msg % (list(estimators), list(preprocessing))
            )
        if not all(prep_check):
            msg = (
                "Too few preprocessing cases to match estimators cases:\n"
                "preprocessing cases: %r\nestimator cases:     %r"
            )
            raise LayerSpecificationError(
                msg % (list(preprocessing), list(estimators))
            )
...
def check_initialized(inst):
    ...
    if not inst.__initialized__:
        msg = (
            "ParallelProcessing is not initialized. Call "
