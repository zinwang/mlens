...
from mlens.utils.checks import (
...
def test_check_ensemble_build_lc_none_no_raise_():
    ...
    FLAG = np.testing.assert_warns(
        LayerSpecificationWarning, check_ensemble_build, lc
    )
...
def test_assert_correct_layer_format_fails_dict_list():
    ...
    np.testing.assert_raises(
        LayerSpecificationError, assert_correct_format, {"a": [OLS()]}, []
    )
...
def test_assert_correct_layer_format_fails_dict_none():
    ...
    np.testing.assert_raises(
        LayerSpecificationError, assert_correct_format, {"a": [OLS()]}, None
    )
...
def test_assert_correct_layer_format_tuple():
    ...
    np.testing.assert_raises(
        LayerSpecificationError, assert_correct_format, OLS(), {"a": [Scale()]}
    )
...
def test_check_initialized_fails():
    ...
    np.testing.assert_raises(
        ParallelProcessingError, check_initialized, Tmp(Lyr(True), 0, 0)
    )
...
def test_check_initialized_fails_fitted():
    ...
    np.testing.assert_raises(
        ParallelProcessingError, check_initialized, Tmp(Lyr(True), 1, 1)
    )
...
def test_check_initialized_warns_fitted():
    ...
    np.testing.assert_warns(
        ParallelProcessingWarning, check_initialized, Tmp(Lyr(False), 1, 1)
    )
